import {
  App, LogLevel, subtype, BotMessageEvent, BlockAction,
} from '@slack/bolt';
import {
 ...
 */
// Your listener function will be called every time an interactive component with
// the action_id "approve_button" is triggered
app.action('approve_button', async ({ ack }) => {
  await ack();
  // Update the message to reflect the action
});

// Your listener function will only be called when the action_id matches 'select_user' AND
// the block_id matches 'assign_ticket'
app.action({ action_id: 'select_user', block_id: 'assign_ticket' },
  async ({ body, client, ack }) => {
    await ack();
    // TODO
    const bodyBlock = body as BlockAction;
    try {
      // Make sure the event is not in a view
      if (bodyBlock.message) {
        await client.reactions.add({
          name: 'white_check_mark',
          timestamp: bodyBlock.message?.ts,
          channel: bodyBlock.channel?.id,
        });
      }
    } catch (error) {
      console.error(error);
